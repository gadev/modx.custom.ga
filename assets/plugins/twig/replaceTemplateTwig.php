...
switch($modx->event->name){
	...
	case 'OnPageNotFound':{
		if(class_exists('Twig_Autoloader')){
			...
			$loader = new Twig_Loader_Filesystem(MODX_BASE_PATH.'/assets/templates/');
	...
	case 'OnAfterLoadDocumentObject':{
		...
		switch(true){
			case file_exists(MODX_BASE_PATH.'assets/templates/'.'tpl-'.$documentObject['template'].'_doc-'.$documentObject['id'].$ext):{
			...
			case file_exists(MODX_BASE_PATH.'assets/templates/doc-'.$documentObject['id'].$ext):{
				$modx->docTemplate = 'doc-'.$documentObject['id'];
				$documentObject['oldTemplate'] = $documentObject['template'];
				$documentObject['template'] = 0;
				break;
			}
			case file_exists(MODX_BASE_PATH.'assets/templates/tpl-'.$documentObject['template'].$ext):{
	...
	case 'OnLoadWebDocument':{
		if(!empty($modx->docTemplate)){
			if($modx->twig instanceof xNop){
				ob_start();
				include(MODX_BASE_PATH.'assets/templates/'.$modx->docTemplate.'.html');
